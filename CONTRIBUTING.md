...
### Updating selectors for a new Spotify version

When Spotify updates its UI and CSS class names, Spicetify's `resources/selectors/*.json` files must be updated to keep themes from breaking.

1. Download the new `xpui.js` from your corresponding platform:
    * **macOS:** `/Applications/Spotify.app/Contents/Resources/Apps/xpui/xpui.js`
    * **Windows:** `%APPDATA%\Spotify\Apps\xpui\xpui.js`
    * **Linux:** `~/.var/app/com.spotify.Client/config/spotify/Apps/xpui/xpui.js`
2. Run our scraper script across the bundle: `node tools/scrape-selectors.mjs path/to/xpui.js > candidate.json`
3. Verify the output, wrap it with `cssVarAliases` copied from `fallback.json` (or updated if needed).
4. Save the finalized JSON as `resources/selectors/1.X.XX.json` matching the Spotify version.
5. Open a PR with only that file changed!
